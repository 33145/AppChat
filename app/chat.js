...
class Chat {
    ...
    this.server.on("connection", (ws) => {
      ...
      ws.send(
        JSON.stringify({
          status: "OK",
          payload: {
            listUser: this.nicknames,
            message: "Welcome to the chat, enjoy :)",
          },
        })
      );
      ws.on("message", (data) => {
        ...
        //mensagem
        if (message.type === "NEW_MESSAGE") this.sendMessage(message, ws);
        ...
        if (message.type === "NEW_USER") {
          ...
          if (this.nicknames.includes(nickname)) {
            this.sendRegError(
              ws,
              "Sorry, the nickname <b>" +
                message.nickname +
                "</b> is already taken. Please choose a different one."
            );
            return;
          } else {
        ...
        if (message.type === "EXIT_ROOM") {
          ...
          if (this.nicknames.includes(author)) {
            ...
            this.sendRegError(
              ws,
              "The nickname <b>" + message.nickname + "</b> disconected. Bye!"
            );
            return;
          }
        }
  ...
  sendAll(_message, ws) {
    ...
    this.clients.forEach((client) => {
      if (client.readyState === 1) client.send(JSON.stringify(env));
    });
  }
  ...
  sendInit(ws) {
    let env = new Response(
      "OK",
      this.nicknames,
      "You are registered as " + ws.username + "."
    );
    ws.send(JSON.stringify(env));
  }
